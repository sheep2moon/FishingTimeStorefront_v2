...
// Shipping actions
export const listShippingMethods = cache(async function listShippingMethods(regionId: string, productIds?: string[]) {
...
export const getProductByHandle = cache(async function (handle: string): Promise<{ product: PricedProduct }> {
    const headers = getMedusaHeaders(["products"]);
...
// export const getProductsList = cache(async function ({ pageParam = 0, queryParams, countryCode }: { pageParam?: number; queryParams?: StoreGetProductsParams; countryCode: string }): Promise<{
...
// export const getHomepageProducts = cache(async function getHomepageProducts({ collectionHandles, currencyCode, countryCode }: { collectionHandles?: string[]; currencyCode: string; countryCode: string }) {
...
export const getCollectionsList = cache(async function (offset: number = 0, limit: number = 100): Promise<{ collections: ProductCollection[]; count: number }> {
...
export const getCollectionByHandle = cache(async function (handle: string): Promise<ProductCollection> {
...
// export const getProductsByCategoryHandle = cache(async function ({ pageParam = 0, handle, countryCode }: { pageParam?: number; handle: string; countryCode: string; currencyCode?: string }): Promise<{
//     response: { products: ProductPreviewType[]; count: number };
//     nextPage: number | null;
// }> {
